...
function detectType(o: Order) {
  const note = (o.notes ?? '').toUpperCase()
  for (const key of DELIVERY_KEYS)
    if (note.includes(`[${key}]`)) return { label: DELIVERY_LABELS[key], color: DELIVERY_COLORS[key], icon: 'truck' as const }
  return { label: o.tableNumber ? `Mesa ${o.tableNumber}` : 'Restaurante', color: '#3b82f6', icon: 'utensils' as const }
}
...
function paymentLabel(o: Order) {
  ...
  if (DELIVERY_KEYS.includes(key)) return (o.notes ?? '').match(/Pago:\s*(\w+)/i)?.[1] ?? '—'
...
export default function AdminReportesPage() {
  ...
  // Totales por método de pago (de los pedidos directos)
  const payTotals = filtered.reduce((acc, o) => {
    const note = (o.notes ?? '').toUpperCase()
    const amt  = o.total ?? 0
    if      (DELIVERY_KEYS.some(k => note.includes(`[${k}]`))) acc.domicilio     += amt
    else if (note.includes('[TARJETA]'))                        acc.tarjeta       += amt
    else if (note.includes('[TRANSFERENCIA]'))                  acc.transferencia += amt
    else                                                        acc.efectivo      += amt
    return acc
  }, { efectivo: 0, tarjeta: 0, transferencia: 0, domicilio: 0 })

  // Ingresos por admin (desde cortes)
  const filteredCortes = cortes.filter(c => inPeriod(c.fin))
  const staffMap = new Map<string, StaffStat>()
  for (const c of filteredCortes) {
    const prev = staffMap.get(c.by) ?? { name: c.by, total: 0, efectivo: 0, tarjeta: 0, transferencia: 0, domicilio: 0, shifts: 0, orders: 0 }
    staffMap.set(c.by, { ...prev, total: prev.total + c.total, efectivo: prev.efectivo + c.efectivo, tarjeta: prev.tarjeta + c.tarjeta, transferencia: prev.transferencia + c.transferencia, domicilio: prev.domicilio + c.domicilio, shifts: prev.shifts + 1, orders: prev.orders + c.orders })
  }
  const staffStats    = [...staffMap.values()].sort((a, b) => b.total - a.total)
  const maxStaffTotal = staffStats[0]?.total || 1

  const byCategory = menu.reduce((acc, m) => { acc[m.category] = (acc[m.category] ?? 0) + 1; return acc }, {} as Record<string, number>)
  ...
  return (
    <div className="min-h-screen md:ml-[240px] md:pt-16" style={{ backgroundColor: S.bg }}>
      ...
      <div className="max-w-[1100px] mx-auto p-4 space-y-5">
        ...
        {loading ? (
          <div className="text-center py-12" style={{ color: S.sub }}>Cargando estadísticas...</div>
        ) : (<>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {([
              ...
              { label: 'Rating',      value: avgRating > 0 ? avgRating.toFixed(1) : '—', color: '#f472b6', icon: 'star' },
            ] as const).map(k => (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {([
              { label: 'Efectivo',      value: payTotals.efectivo,      color: '#22c55e' },
              { label: 'Tarjeta',       value: payTotals.tarjeta,       color: '#3b82f6' },
              { label: 'Transferencia', value: payTotals.transferencia, color: '#a855f7' },
              { label: 'Domicilio',     value: payTotals.domicilio,     color: '#f97316' },
            ]).map(({ label, value, color }) => (
              <div key={label} className="rounded-2xl p-3 text-center" style={{ backgroundColor: S.card, border: `1px solid ${S.border}` }}>
                <p className="text-xs font-bold" style={{ color: S.sub }}>{label}</p>
                <p className="text-lg font-black mt-0.5" style={{ color }}>${value.toFixed(0)}</p>
              </div>
            ))}
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-2xl p-4 flex items-center gap-4" style={{ backgroundColor: S.card, border: `1px solid ${S.border}` }}>
              <div className="w-10 h-10 rounded-2xl flex items-center justify-center shrink-0" style={{ backgroundColor: 'rgba(59,130,246,0.12)', color: '#3b82f6' }}>
            ...
            <div className="rounded-2xl p-4 flex items-center gap-4" style={{ backgroundColor: S.card, border: `1px solid ${S.border}` }}>
              <div className="w-10 h-10 rounded-2xl flex items-center justify-center shrink-0" style={{ backgroundColor: 'rgba(249,115,22,0.12)', color: '#f97316' }}>
                  ...
                  <div key={st.name} className="px-5 py-4 space-y-2.5">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full flex items-center justify-center font-black text-sm shrink-0"
                        style={{ background: i === 0 ? 'linear-gradient(135deg,#f59e0b,#d97706)' : S.elevated, color: i === 0 ? '#000' : S.sub }}>
                        {i + 1}
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <p className="font-bold text-sm truncate" style={{ color: S.text }}>{st.name}</p>
                          <span className="text-xs px-1.5 py-0.5 rounded-full shrink-0" style={{ backgroundColor: S.elevated, color: S.sub }}>
                            {st.shifts} turno{st.shifts !== 1 ? 's' : ''} · {st.orders} pedidos
                          </span>
                        </div>
                        <div className="h-1.5 rounded-full mt-1.5" style={{ backgroundColor: S.elevated }}>
                          <div className="h-full rounded-full" style={{ width: `${(st.total / maxStaffTotal) * 100}%`, backgroundColor: 'var(--ad-accent)' }} />
                        </div>
                      </div>
                    ...
                    <div className="grid grid-cols-4 gap-1.5 ml-11">
                      {([
                        ...
                        <div key={label} className="rounded-lg p-1.5 text-center" style={{ backgroundColor: S.elevated }}>
          ...
          {/* Grid: platillos + categorías + reseñas */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="rounded-2xl" style={{ backgroundColor: S.card, border: `1px solid ${S.border}` }}>
              ...
              <div className="p-5 space-y-3">
                {topMenu.length === 0 ? <p className="text-sm text-center" style={{ color: S.sub }}>Sin datos</p>
                : topMenu.map((item, i) => {
                  ...
                  return (
                    <div key={item.id} className="flex items-center gap-3">
                      <span className="w-6 h-6 shrink-0 rounded-full flex items-center justify-center text-xs font-black"
                        style={{ backgroundColor: i === 0 ? 'rgba(245,158,11,0.18)' : S.elevated, color: i === 0 ? '#f59e0b' : S.sub }}>{i + 1}</span>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-bold truncate" style={{ color: S.text }}>{item.name}</p>
                        <div className="h-1.5 rounded-full mt-1" style={{ backgroundColor: S.elevated }}>
                          <div className="h-full rounded-full" style={{ width: `${(item.likes / maxLikes) * 100}%`, backgroundColor: 'var(--ad-accent)' }} />
                        </div>
                      </div>
              ...
              <div className="p-5 space-y-3">
                {Object.entries(byCategory).map(([cat, count]) => (
                  <div key={cat} className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <div className="h-1.5 w-24 rounded-full" style={{ backgroundColor: S.elevated }}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="inline-flex items-center gap-0.5" style={{ color: '#fbbf24' }}>
                            {Array.from({ length: r.rating }).map((_, s) => <Icon key={s} name="star" size={11} />)}
                          </span>
                        </div>
